package xyz.server.scrcpy.model;
...
public class ByteUtils {
    ...
    public static int bytesToInt(byte[] bytes) {
        ...
        // First byte unmasked (sign-extended), matching the previous BigInteger-based signed
        // two's-complement semantics for high-bit-set input.
        return (bytes[0] << 24)
                | ((bytes[1] & 0xFF) << 16)
                | ((bytes[2] & 0xFF) << 8)
                | (bytes[3] & 0xFF);
